...
// built on the AstrodomeCitationsPlugin by Erin Bell

class MBCitationsPlugin extends Omeka_Plugin_AbstractPlugin
{
	...
	/* Specification:
	** BibliographicCitation, site title (Max-Bense-Collection), accessed [date], [URL].
    ...
    */	
	...
	$bibdata = metadata($item, array('Dublin Core Extended', 'BibliographicCitation')) ? metadata($item, array('Dublin Core Extended', 'BibliographicCitation')).', ' : null;
...
//		$citation = $creator.$title.$date.'. '.$provenance.'<em>'.$publication.'</em>, accessed '.$today.', '.$url;
	$citation = $bibdata . '<em>' . $publication . '</em>, accessed ' . $today . ', ' . $url . '.';
